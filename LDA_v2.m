function LDA_v2(data, class_mu, mu, Colors, Odorants, files, LDA_title)
all_data = reshape(permute(data,[2 3 1]),size(data,2)*size(data,3),size(data,1));
Sw = zeros(size(files{1},1));
Sb = zeros(size(files{1},1));
...
for cycle_lambda = 1:num_graphs
    ...
    lambda = (cycle_lambda-1)/(num_graphs-1);     % Vanilla LDA when lambda is zero; spherical LDA when lambda is one
...
%%%%%%%%%%%%%%%Specify lambda for master figure%%%%%%%%%%%%%%%

lambda = 0;     % Vanilla LDA when lambda is zero; spherical LDA when lambda is one
mat_inv = ((1-lambda)*Sw+lambda*eye(size(Sw)))\Sb;
[eigvec, eigval] = eig(mat_inv,'vector');
[eigval, idx] = sort(eigval,'descend');
eigvec = real(eigvec(:,idx));
