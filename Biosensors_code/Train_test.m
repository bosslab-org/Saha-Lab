function [bin_preds, trial_preds] = Train_test(data,normtype,train_trials)

% train_data_mean = permute(mean(data(:,:,train_trials,:),3),[1 2 4 3]);
train_data_mean = permute(mean(data(:,:,train_trials,:),[2,3]),[1 2 4 3]);
...
for cycle_trials = 1:size(test_data,3)    
    ...
    for cycle_stimuli= 1:size(test_data,4)
        bin_norm = squeeze(vecnorm(test_data_temp(:,:,cycle_stimuli)-train_data_mean,normtype,1)); % Specified norm along dimension 1
