...
write_path = ['RMS/' filepath3 '/Day_files'];
trial = 3;
...
for cycle_odors = 1:length(files)        
...
%                    data_filt(cycle_tetrodes,cycle_channels,cycle_trials,:) =  filtfilt(b,a,notch_filter(amplifier_data(channels_idx(tetrode_channels(cycle_channels)),(sample_rate+1:trimmed_trial_length)), sample_rate, notch_filter_frequency, 10));
                  data_filt(cycle_tetrodes,cycle_channels,cycle_trials,:) =  notch_filter(amplifier_data(channels_idx(tetrode_channels(cycle_channels)),(sample_rate+1:trimmed_trial_length)), sample_rate, notch_filter_frequency, 10);
                else
...
%%%%%%%%%%%%%%Save data within odor experiment folder%%%%%%%%%%%%%%       
    ...
    if ~exist([filepath '/' write_path '/' date '/Position_' Position])
        mkdir([filepath '/' write_path '/' date '/Position_' Position]);
    end
    fprintf(1, '\nSaving %s data... \n', Odor);    
    save([filepath '/' write_path '/' date '/Position_' Position '/' Odor], [Odor '_data_filt'], 'stim_on','stim_off','total_time','sample_rate');
