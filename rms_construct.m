function rms_construct(stimulus, data, exp_pars, rms_window, smooth_window, bin_size, time_window, filepath, exp_path)

% readpath = [filepath '/MAT_files/' date 'Position_' position];
writepath = [filepath '/RMS_files/']; % date '/Position_' position];
filename = ['R' num2str(rms_window) '_S' num2str(smooth_window) '_b' num2str(bin_size)];

stim_on = exp_pars(1);
stim_off = exp_pars(2);
total_time = exp_pars(3);
sample_rate = exp_pars(5);

preprocess_data = @(x) movmean(sqrt(movmean(x.^ 2, rms_window, 4)),smooth_window,4);
data = preprocess_data(data(:,:,:,(stim_on+time_window(1))*sample_rate+1:(stim_on+time_window(2))*sample_rate)) - mean(preprocess_data(data(:,:,:,(stim_on-2)*sample_rate+1:stim_on*sample_rate)),[3 4]);                
data = permute(mean(reshape(permute(data,[4 3 1 2]),(sample_rate*bin_size)/1000,size(data,4)/((sample_rate*bin_size)/1000),size(data,3),size(data,1),size(data,2)),[1 5]),[4 3 2 1]);

if exist([writepath '/' filename '.mat'],'file') %check for file existence
    load([writepath '/' filename], [stimulus(1:end-3) '_RMS'], 'experiment')
    ...
    if exp==1 && ~exist([stimulus(1:end-3) '_RMS'])
        ...
        save([writepath '/' filename], [stimulus(1:end-3) '_RMS'],'-append');
    elseif exp~=1 && exist([stimulus(1:end-3) '_RMS'])
        ...
        eval([stimulus(1:end-3) '_RMS = data;'])
        save([writepath '/' filename], [stimulus(1:end-3) '_RMS'], 'experiment','-append');
    elseif exp==1 && exist([stimulus(1:end-3) '_RMS'])
        data = cat(1,eval([stimulus(1:end-3) '_RMS']),data);
        eval([stimulus(1:end-3) '_RMS = data;'])
        save([writepath '/' filename], [stimulus(1:end-3) '_RMS'], 'experiment','-append');
    ...
    save([writepath '/' filename], [stimulus(1:end-3) '_RMS'],'experiment', 'stim_on','stim_off','total_time','sample_rate');
end
